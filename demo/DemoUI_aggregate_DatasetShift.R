...
X <- cbind(X1, X2)
Y <- rbinom(n = n, size = 1,
            prob = exp(theta0 + X1 * theta1 + X2 * theta2) /
              (1 + exp(theta0 + X1 * theta1 + X2 * theta2)))
test.data <- data.frame(response = Y,
                        covariate = X)
...
Y_shift <- rbinom(n = N, size = 1,
                  prob = exp(theta0 + X1_shift * theta1 + X2_shift * theta2) /
                    (1 + exp(theta0 + X1_shift * theta1 + X2_shift * theta2)))
