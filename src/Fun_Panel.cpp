...
// [[Rcpp::export]]
arma::cube get_effVinv_panel_rcpp0(const arma::mat & X,
                                  const arma::mat & Y,
                                  const double & h,
                                  std::string kernel = "K2_Ep") {
  ...
  arma::cube effV_inv(n_t, n_t, n_n);
    ...
    for (arma::uword j = 0; j < n_t; ++j) {
      ...
      for (arma::uword j_F = 0; j_F < n_t; ++j_F) {
        ...
        for (arma::uword i_F = 0; i_F < n_n; ++i_F) {
          ...
          double u = (X(i_F, j_F) - target_x_j) * inv_h;
          double k_val = k_func(u);
    ...
    for (arma::uword j = 0; j < n_t; ++j) {
      ...
      for (arma::uword k = 0; k < n_t; ++k) {
        ...
        if (j == k) {
          ...
          effV_inv(j, k, i) = p_ij(j) * (1 - p_ij(j));

        } else {

          effV_inv(j, k, i) = p_ij(j) * p_ij(k);

          double c_ijk_Ntor = 0.0;
          double c_ijk_Dtor = 0.0;

          for (arma::uword i_c = 0; i_c < n_n; ++i_c) {

            double u_j = (X(i_c, j) - target_x_j) * inv_h;
            double u_k = (X(i_c, k) - target_x_k) * inv_h;
            double k_val_jk = k_func(u_j) * k_func(u_k);

            c_ijk_Dtor += k_val_jk;
            c_ijk_Ntor += Y(i_c, j) * Y(i_c, k) * k_val_jk;
          }

          double p_ijk_Dtor = p_ij(j) * p_ij(k);

          if ((std::abs(c_ijk_Dtor) > 1e-10) && (std::abs(p_ijk_Dtor) > 1e-10)) {
            rho_sum += (c_ijk_Ntor / c_ijk_Dtor - p_ijk_Dtor) / p_ijk_Dtor;
            valid_c_count += 1.0;
          }
        }
      }
  ...
  rho_sum /= valid_c_count;
  effV_inv *= rho_sum;

  for (arma::uword i = 0; i < n_n; ++i) {

    effV_inv.slice(i).diag() /= rho_sum;
    effV_inv.slice(i) = arma::inv(effV_inv.slice(i));
  }
